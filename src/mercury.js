/*!
 * window.Mercury Editor is a CoffeeScript and jQuery based WYSIWYG editor.  Documentation and other useful information can be
 ...
 * require jquery_ujs
 *
 * Add any requires for the support libraries that integrate nicely with window.Mercury Editor.
 * require mercury/support/history
 *
 * Require window.Mercury Editor itself.
 ...
 *
 * Add all requires for plugins that extend or change the behavior of window.Mercury Editor.
 * require mercury/plugins/save_as_xml/plugin.js
 *
 * Require any files you want to use that either extend, or change the default window.Mercury behavior.
 * require mercury_overrides
 */
window.window.Mercury = {

  // # window.Mercury Configuration
  config: {
    // ## Toolbars
    //
    ...
    // `window.Mercury.Toolbar.Button.contexts` and `window.Mercury.Toolbar.ButtonGroup.contexts`
    toolbars: {
      primary: {
    ...
    //
    // attribute: window.Mercury identifies editable regions by a data-mercury attribute.  This attribute has to be added in
    // your HTML in advance, and is the only real code/naming exposed in the implementation of window.Mercury.  To allow this
    ...
    // server upon saving.  These attributes, when applied to a window.Mercury region element, will be automatically serialized
    ...
    //
    // handler: You can use false to let window.Mercury handle it for you, or you can provide a handler function that can
    // modify the response from the server.  This can be useful if your server doesn't respond the way window.Mercury expects.
    ...
    // ## Localization / I18n
    //
    // Include the .locale files you want to support when loading window.Mercury.  The files are always named by the language,
    ...
    // type.
    //
    //     window.Mercury.Regions.Full.actions.htmlEditor = function() {}
    //
    ...
    behaviors: {
      ...
      htmlEditor: function() { window.Mercury.modal('/mercury/modals/htmleditor.html', { title: 'HTML Editor', fullHeight: true, handler: 'htmlEditor' }); }
      },
    ...
    //       window.Mercury.modal(null, {title: 'Saving', closeButton: true, content: content})
    //     }
    //
    // This is a nice way to add functionality, when the behaviors aren't region specific.  These can be triggered by a
    // button, or manually with `window.Mercury.trigger('action', {action: 'barrelRoll'})`
    globalBehaviors: {
    ...
    // tokens are typically stored in the rendered DOM.  By default, window.Mercury will look for the Rails specific meta tag,
    ...
    //
    // window.Mercury will hijack links and forms that don't have a target set, or the target is set to _self and will set it
    // to _parent.  This is because the target must be set properly for window.Mercury to not get in the way of some
    // functionality, like proper page loads on form submissions etc.  window.Mercury doesn't do this to links or forms that
    ...
    // ## Pasting & Sanitizing
    //
    // When pasting content into window.Mercury it may sometimes contain HTML tags and attributes.  This markup is used to
    ...
    // ## Injected Styles
    //
    // window.Mercury tries to stay as much out of your code as possible, but because regions appear within your document we
